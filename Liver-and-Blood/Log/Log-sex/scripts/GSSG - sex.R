...
qtlscan_LiverGSSG <- scan1(genoprobs = probs, pheno = pheno["logLiverGSSG"], kinship = kinship_loco, addcovar = sex, cores=10)
perm_LiverGSSG <- scan1perm(genoprobs = probs, pheno = pheno["logLiverGSSG"], addcovar = sex, n_perm = 1000, cores=10)
...
#using gmap (cM)
find_peaks(scan1_output = qtlscan_LiverGSSG, map = R01_GSH_DO_QTLdata$gmap, threshold = summary(perm_LiverGSSG, alpha = 0.1), prob = 0.95, expand2markers = FALSE)
#using pmap (cM)
find_peaks(scan1_output = qtlscan_LiverGSSG, map = R01_GSH_DO_QTLdata$pmap, threshold = summary(perm_LiverGSSG, alpha = 0.1), prob = 0.95, expand2markers = FALSE)
...
#For Liver GSSG --- Chromosome 2
  ...
  #using gmap (cM)
  chr = 2
  coef_blup_LiverGSSG_chr2 <- scan1blup(genoprobs =  probs[,chr], pheno = pheno["logLiverGSSG"], kinship = kinship_loco[[chr]], addcovar = sex, cores = 10)
  ...
  variants_LiverGSSG_chr2 <- query_variants(chr, 100, 110)
  out_snps_LiverGSSG_chr2 <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["logLiverGSSG"], kinship = kinship_loco[[chr]], addcovar = sex, query_func = query_variants,
  ...
  LiverGSSG_Genes_MGI_chr2 <- query_genes_mgi(chr = chr, start = 100, end = 110)
...
out_gwas_LiverGSSG <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["logLiverGSSG"], kinship = kinship_loco, addcovar = sex, query_func=query_variants, cores=10)
par(mar=c(4.1, 4.1, 2.6, 2.6))
