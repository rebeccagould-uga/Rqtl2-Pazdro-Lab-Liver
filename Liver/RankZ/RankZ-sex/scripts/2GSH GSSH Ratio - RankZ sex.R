...
qtlscan_Liver2GSH_GSSGRatio<- scan1(genoprobs = probs, pheno = pheno["zLiver2GSH_GSSGRatio"], kinship = kinship_loco, addcovar = sex, cores=10)
perm_Liver2GSH_GSSGRatio <- scan1perm(genoprobs = probs, pheno = pheno["zLiver2GSH_GSSGRatio"], addcovar = sex, n_perm = 1000, cores=10)

#set working directory
pdf(file = "2GSH GSSG Ratio QTL Results - RankZ sex.pdf")
##NOW SAVING ALL PLOTS AND TABLES ONTO A PDF##
  ...
  write_xlsx(list("2GSH_GSSGRatio gmap (cM)" = gmap_peaks2GSH_GSSGRatio,
             ...
             "2GSH GSSG Ratio Peaks - RankZ sex.xlsx")
...
#For Liver 2GSH/GSSG --- Chromosome 16
  ...
  #using gmap (cM)
  chr = 16
  coef_blup_Liver2GSH_GSSGRatio_chr16 <- scan1blup(genoprobs =  probs[,chr], pheno = pheno["zLiver2GSH_GSSGRatio"], kinship = kinship_loco[[chr]], addcovar = sex, cores = 10)
  ...
  out_snps_Liver2GSH_GSSGRatio_chr16 <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["zLiver2GSH_GSSGRatio"], kinship = kinship_loco[[chr]], addcovar = sex, query_func = query_variants,
...
pdf(file = "2GSH GSSG Ratio GWAS - RankZ sex.pdf")
out_gwas_Liver2GSH_GSSGRatio <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["zLiver2GSH_GSSGRatio"], kinship = kinship_loco, addcovar = sex, query_func=query_variants, cores=10)
par(mar=c(4.1, 4.1, 2.6, 2.6))
