...
#LIVER HISTOLOGY MAPPING - BALLOONING
...
#For Ballooning --- Chromosome 2
  ...
  out_snps_Ballooning_chr2 <- scan1snps(genoprobs = probs, map = R01_Ballooning_DO_QTLdata$pmap, pheno = pheno["zBallooning"], kinship = kinship_loco[[chr]], addcovar = sexgen, query_func = query_variants,
...
out_gwas_Ballooning <- scan1snps(genoprobs = probs, map = R01_Ballooning_DO_QTLdata$pmap, pheno = pheno["zBallooning"], kinship = kinship_loco, addcovar = sexgen, query_func=query_variants, cores=10)
par(mar=c(4.1, 4.1, 2.6, 2.6))
