...
qtlscan_LiverNADH <- scan1(genoprobs = probs, pheno = pheno["zLiverNADH"], kinship = kinship_loco, addcovar = sex, cores=10)
perm_LiverNADH <- scan1perm(genoprobs = probs, pheno = pheno["zLiverNADH"], addcovar = sex, n_perm = 1000, cores=10)

#set working directory
pdf(file = "NADH QTL Results - RankZ sex.pdf")
...
#For Liver NADH --- Chromosome 14 
  ...
  #using gmap (cM)
  chr = 14
  coef_blup_LiverNADH_chr14 <- scan1blup(genoprobs =  probs[,chr], pheno = pheno["zLiverNADH"], kinship = kinship_loco[[chr]], addcovar = sex, cores = 10)
...
pdf(file = "NADH GWAS - RankZ sex.pdf")
out_gwas_LiverNADH <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["zLiverNADH"], kinship = kinship_loco, addcovar = sex, query_func=query_variants, cores=10)
par(mar=c(4.1, 4.1, 2.6, 2.6))
