...
qtlscan_LiverNADP <- scan1(genoprobs = probs, pheno = pheno["zLiverNADP"], kinship = kinship_loco, addcovar = sex, cores=10)
perm_LiverNADP <- scan1perm(genoprobs = probs, pheno = pheno["zLiverNADP"], addcovar = sex, n_perm = 1000, cores=10)

#set working directory
pdf(file = "NADP QTL Results - RankZ sex.pdf")
...
#For Liver NADP --- Chromosome 8
  ...
  #using gmap (cM)
  chr = 8
  coef_blup_LiverNADP_chr8 <- scan1blup(genoprobs =  probs[,chr], pheno = pheno["zLiverNADP"], kinship = kinship_loco[[chr]], addcovar = sex, cores = 10)
  ...
  variants_LiverNADP_chr8 <- query_variants(chr, 60.5, 66.5)
  out_snps_LiverNADP_chr8 <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["zLiverNADP"], kinship = kinship_loco[[chr]], addcovar = sex, query_func = query_variants,
                                       chr = chr, start = 60.5, end = 66.5, keep_all_snps = TRUE)
  plot_snpasso(out_snps_LiverNADP_chr8$lod, out_snps_LiverNADP_chr8$snpinfo, main = "Liver NADP SNPs")
  
  LiverNADP_Genes_MGI_chr8 <- query_genes_mgi(chr = chr, start = 60.5, end = 66.5)
...
pdf(file = "NADP GWAS - RankZ sex.pdf")
out_gwas_LiverNADP <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["zLiverNADP"], kinship = kinship_loco, addcovar = sex, query_func=query_variants, cores=10)
par(mar=c(4.1, 4.1, 2.6, 2.6))
