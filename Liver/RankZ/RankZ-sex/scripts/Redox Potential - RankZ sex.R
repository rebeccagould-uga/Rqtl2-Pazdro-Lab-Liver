...
#LIVER QTL MAPPING - Redox Potential GSSG/2GSH
...
qtlscan_LiverRedoxPotentialGSSG2GSH<- scan1(genoprobs = probs, pheno = pheno["zLiverRedoxPotentialGSSG2GSH"], kinship = kinship_loco, addcovar = sex, cores=10)
perm_LiverRedoxPotentialGSSG2GSH <- scan1perm(genoprobs = probs, pheno = pheno["zLiverRedoxPotentialGSSG2GSH"], addcovar = sex, n_perm = 1000, cores=10)
...
#For Liver Redox Potential GSSG/2GSH --- Chromosome 16
  ...
  #using gmap (cM)
  chr = 16
  coef_blup_LiverRedoxPotentialGSSG2GSH_chr16 <- scan1blup(genoprobs =  probs[,chr], pheno = pheno["zLiverRedoxPotentialGSSG2GSH"], kinship = kinship_loco[[chr]], addcovar = sex, cores = 10)
  ...
  out_snps_LiverRedoxPotentialGSSG2GSH_chr16 <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["zLiverRedoxPotentialGSSG2GSH"], kinship = kinship_loco[[chr]], addcovar = sex, query_func = query_variants,
...
out_gwas_LiverRedoxPotentialGSSG2GSH <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["zLiverRedoxPotentialGSSG2GSH"], kinship = kinship_loco, addcovar = sex, query_func=query_variants, cores=10)
par(mar=c(4.1, 4.1, 2.6, 2.6))
