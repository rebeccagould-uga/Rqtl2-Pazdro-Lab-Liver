...
qtlscan_LiverNADH <- scan1(genoprobs = probs, pheno = pheno["logLiverNADH"], kinship = kinship_loco, addcovar = sexgen, cores=10)
perm_LiverNADH <- scan1perm(genoprobs = probs, pheno = pheno["logLiverNADH"], addcovar = sexgen, n_perm = 1000, cores=10)

par(mar=c(4.1, 4.1, 2.6, 2.6))
threshold_LiverNADH = summary(perm_LiverNADH, alpha = c(0.35, 0.2, 0.1, 0.05))
plot_scan1(x = qtlscan_LiverNADH, map = R01_GSH_DO_QTLdata$gmap,  main = "Genome Scan for Liver NADH", ylim = c(0,11))
abline(h = threshold_LiverNADH, col = c("orange", "purple", "red", "blue"), lwd = 2)
#using gmap (cM)
...
#using gmap (cM)
chr = 14
coef_blup_LiverNADH_chr14 <- scan1blup(genoprobs =  probs[,chr], pheno = pheno["logLiverNADH"], kinship = kinship_loco[[chr]], addcovar = sexgen, cores = 10)
...
#using pmap (Mbp)
chr = 14
peaksNADH <- find_peaks(scan1_output = qtlscan_LiverNADH, map = R01_GSH_DO_QTLdata$pmap, threshold = 6, drop = 1.5)
...
out_snps_LiverNADH_chr14 <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["logLiverNADH"], kinship = kinship_loco[[chr]], addcovar = sexgen, query_func = query_variants,
...
out_gwas_LiverNADH <- scan1snps(genoprobs = probs, map = R01_GSH_DO_QTLdata$pmap, pheno = pheno["logLiverNADH"], kinship = kinship_loco, addcovar = sexgen, query_func=query_variants, cores=10)
par(mar=c(4.1, 4.1, 2.6, 2.6))
